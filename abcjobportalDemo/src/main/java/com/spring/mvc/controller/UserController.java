...
import com.spring.mvc.servic.AdminService;
import com.spring.mvc.servic.BulkMailService;
import com.spring.mvc.servic.ForgotPassEmailService;
import com.spring.mvc.servic.MailService;
import com.spring.mvc.servic.UserService;

@Controller
public class UserController {

	@Autowired//To call the userservice
	private UserService userService;
	@Autowired//To call the mail servic
...
//If user information can not get it will show error
	...
	//For checking email is it in database or not and send mail to the definite email
	//start
		...
		//to ost form and send regis confirmation
		@PostMapping("/adduser")
		public String register1(@ModelAttribute("user") User user, Model model, HttpServletRequest request,
				String emailToRecipient, String userName,String EmailID) {
			emailToRecipient = request.getParameter("email");
			userName = request.getParameter("name");
		...
		@RequestMapping(value = "/emailf", method = RequestMethod.POST)
		public String sendEmail( HttpServletRequest request,
			...
			bulkMailService.sendMails(receiver, subject, body);
